package com.example.calculator.ui.view.viewmodel
...
import com.example.calculator.utils.Constance
...
class CalculatorViewModel : ViewModel() {
    ...
    private fun enterNumber(number: Int) {
        if (state.operation == null) {
            if (state.number1.length >= Constance.MAX_NUM_LENGTH) {
                return
            }
            state = state.copy(number1 = state.number1 + number)
            return
        }
        if (state.number2.length >= Constance.MAX_NUM_LENGTH) {
            return
        }
